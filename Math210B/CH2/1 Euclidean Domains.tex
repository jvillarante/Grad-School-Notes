...
\begin{proof}
    If $I$ is the zero ideal, then $I=(0)$. Suppose $I\neq 0$. Then let $d$ be a nonzero element of $I$ with minimal norm (such a $d$ exists since the set $\set{N(a) : A \in I}$ is a nonempty set of integers bounded below and by the well-ordering principal there must be a minimal norm attained by an element of $I$). Clearly, $(d) \subseteq I$ since $d\in I$. To show $I \subseteq (d)$ let $a \in I$ and use the division algorithm to write
    $$a = qd +r$$
