...
\begin{corollary}
    \begin{enumerate}
        \item Suppose $V$ is a finite basis with $n$ elements. Any set of linearly independent vectors has at most $n$ elements.
        \item If $V$ has some finite basis, then any two bases of $V$ have the same number of elements.
    \end{enumerate}
\end{corollary}

\begin{definition}[Dimension of a Vector Space] \leavevmode \\
    If $V$ is a vector space over $F$ with a finite basis, then the cardinality of any basis is called the dimension of $V$, denoted $\dim V$.
\end{definition}